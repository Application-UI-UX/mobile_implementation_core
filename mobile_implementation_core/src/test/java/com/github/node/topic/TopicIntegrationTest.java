...
import com.github.internal.message.definition.MessageDefinitionReflectionProvider;
import com.github.internal.message.topic.TopicMessageFactory;
...
public class TopicIntegrationTest extends RosTest {
   ...
   * 
   * @see <a
   *      href="http://answers.ros.org/question/3591/mobile_implementation_core-subscriber-unreliable">bug
   *      report</a>
   * 
  ...
  private final class Listener implements MessageListener<mobile_implementation_core_test_msgs.TestHeader> {
    private final CountDownLatch latch = new CountDownLatch(10);

    private mobile_implementation_core_test_msgs.TestHeader lastMessage;

    @Override
    public void onNewMessage(mobile_implementation_core_test_msgs.TestHeader message) {
  ...
  public void testHeader() throws InterruptedException {
    nodeMainExecutor.execute(new AbstractNodeMain() {
      ...
      public void onStart(final ConnectedNode connectedNode) {
        final Publisher<mobile_implementation_core_test_msgs.TestHeader> publisher =
            connectedNode.newPublisher("foo", mobile_implementation_core_test_msgs.TestHeader._TYPE);
        connectedNode.executeCancellableLoop(new CancellableLoop() {
          @Override
          public void loop() throws InterruptedException {
            mobile_implementation_core_test_msgs.TestHeader message =
                connectedNode.getTopicMessageFactory().newFromType(mobile_implementation_core_test_msgs.TestHeader._TYPE);
    ...
    nodeMainExecutor.execute(new AbstractNodeMain() {
      ...
      @Override
      public void onStart(ConnectedNode connectedNode) {
        Subscriber<mobile_implementation_core_test_msgs.TestHeader> subscriber =
            connectedNode.newSubscriber("foo", mobile_implementation_core_test_msgs.TestHeader._TYPE);
