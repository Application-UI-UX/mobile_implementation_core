...
public class PassthroughTestNode extends AbstractNodeMain {
  ...
  public GraphName getDefaultNodeName() {
    return GraphName.of("rosmobile/passthrough_test_node");
  }
  ...
  public void onStart(final ConnectedNode connectedNode) {
    ...
    final Publisher<rosmobile_test_msgs.TestHeader> pub_header =
        connectedNode.newPublisher("test_header_out", rosmobile_test_msgs.TestHeader._TYPE);
    MessageListener<rosmobile_test_msgs.TestHeader> header_cb = new MessageListener<rosmobile_test_msgs.TestHeader>() {
      @Override
      public void onNewMessage(rosmobile_test_msgs.TestHeader m) {
        m.setOrigCallerId(m.getCallerId());
        m.setCallerId(connectedNode.getName().toString());
        pub_header.publish(m);
      }
    };
    Subscriber<rosmobile_test_msgs.TestHeader> testHeaderSubscriber =
        connectedNode.newSubscriber("test_header_in", "rosmobile_test_msgs/TestHeader");
    testHeaderSubscriber.addMessageListener(header_cb);

    // TestComposite pass through
    final Publisher<rosmobile_test_msgs.Composite> pub_composite =
        connectedNode.newPublisher("composite_out", "rosmobile_test_msgs/Composite");
    MessageListener<rosmobile_test_msgs.Composite> composite_cb = new MessageListener<rosmobile_test_msgs.Composite>() {
      @Override
      public void onNewMessage(rosmobile_test_msgs.Composite m) {
        pub_composite.publish(m);
      }
    };
    Subscriber<rosmobile_test_msgs.Composite> compositeSubscriber =
        connectedNode.newSubscriber("composite_in", "rosmobile_test_msgs/Composite");
